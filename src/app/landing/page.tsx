...
import CountdownTimer, { LAUNCH_DATE } from "@/components/CountdownTimer";
...
const LAUNCH_DATE_TYPE = new Date(LAUNCH_DATE).getTime();
...
export default function LandingPage() {
    ...
    const [isReleased, setIsReleased] = useState(() => Date.now() >= LAUNCH_DATE_TYPE );
    ...
    return (
        <div ref={containerRef} className="relative min-h-screen w-full overflow-x-hidden bg-slate-950 text-white font-sans selection:bg-blue-500/30 selection:text-blue-200">
            ...
            {/* 1. FIXED COUNTDOWN OVERLAY GATE: Blurs everything beneath it until launch hits */}
            {!isReleased && (
                <div className="fixed inset-0 z-50 flex flex-col items-center justify-center px-4 bg-slate-950/40 backdrop-blur-xl animate-fade-in select-none">
                    <div className="w-full max-w-4xl text-center flex flex-col items-center space-y-4 px-4">
                        <div className="text-[11px] font-mono tracking-widest uppercase text-cyan-400 font-semibold bg-cyan-950/40 px-4 py-1.5 rounded-full border border-cyan-800/30 shadow-inner">
                            Brixta Embedding Launch Countdown
                        </div>

                        {/* Streamed Callback passes straight into the timer block below */}
                        <CountdownTimer onComplete={() => setIsReleased(true)} />
            ...
            {/* 2. MAIN CORE CONTENT PACK: Cleanly unblurs when overlay collapses */}
            <div className={`transition-all duration-1000 ease-in-out ${!isReleased ? "blur-md pointer-events-none opacity-40" : "blur-0 opacity-100"}`}>
                <NavBar />
